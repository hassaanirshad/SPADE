...
public class AuditProfile{
	...
	private final Map<String, Perioder> kernelSyscallPerioder = new HashMap<String, Perioder>();
	...
	public void kernelSyscallStart(String syscallName){
		syscallName = String.valueOf(syscallName).toLowerCase();
		Perioder perioder = kernelSyscallPerioder.get(syscallName);
		if(perioder == null){
			perioder = new Perioder();
			kernelSyscallPerioder.put(syscallName, perioder);
		}
		perioder.start();
	}
	
	public void kernelSyscallEnd(String syscallName){
		syscallName = String.valueOf(syscallName).toLowerCase();
		Perioder perioder = kernelSyscallPerioder.get(syscallName);
		if(perioder != null){
			perioder.end();
	...
	public void kernelSyscallEndException(String syscallName, Exception e){
		syscallName = String.valueOf(syscallName).toLowerCase();
		Perioder perioder = kernelSyscallPerioder.get(syscallName);
		if(perioder != null){
			perioder.end();
	...
	public void shutdown(){
		...
		for(Map.Entry<String, Perioder> entry : kernelSyscallPerioder.entrySet()){
			String key = entry.getKey();
			Perioder perioder = entry.getValue();
			if(key != null && perioder != null){
				String syscallPerioderPath = ProfileConfig.getOutFilePath("audit-kernel-syscall-" + key + "-perioder.csv");
				perioder.toFile(syscallPerioderPath);
			}
		}
